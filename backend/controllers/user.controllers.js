...
export const AllSavedPosts = async (req, res) => {
    try {
      const userId = req.user._id.toString(); 
      if (!userId) return res.status(400).json({ error: "User id is required" });
      ...
      // Populate the savedPosts field with the full posts, including user and comments user
      await user.populate({
        path: 'savedPosts',
        populate: [
          {
      ...
      console.error("Error in user controllers: ", error.message);
      res.status(500).json({ error: error.message || "Network error" });
    }
  };
